...
def ocr_app():
    st.title("OCR App")
    st.write("This app extracts text from the camera feed in real-time.")
    ...
    def process_ocr():
        ...
        while True:
            ...
            cv2.imshow("Camera - Text Recognition", frame_resized)

            key = cv2.waitKey(1) & 0xFF
            if key == ord('s'):
                st.write("Processing frame for text recognition. Please wait.")
                recognized_text = read_text_from_frame(frame_resized)
                if recognized_text.strip():
                    st.write(f"Recognized Text: {recognized_text}")
                    speak("Recognized text is: " + recognized_text)
                else:
                    st.write("No text recognized.")
    ...
    if st.button("Start OCR Stream"):
        st.write("Starting OCR...")
        process_ocr()

if __name__ == "__main__":
    st.set_page_config(page_title="Camera and OCR App", layout="wide")
    st.title("Real-Time Camera and OCR App")

    app_mode = st.sidebar.selectbox("Choose the app mode", ["Camera", "OCR"])
